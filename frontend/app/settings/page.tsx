    ...
    <AppLayout activeWorkspaceId={activeWorkspaceId} onWorkspaceChange={setActiveWorkspaceId}>
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-5 pb-6 border-b border-white/[0.08]">
          ...
          <div className="flex items-center gap-3 bg-white/[0.03] border border-white/[0.08] px-5 py-2.5 rounded-full shrink-0">
            <span
              className={`w-2.5 h-2.5 rounded-full ${
                wsConnected ? "bg-emerald-400" : "bg-white/20"
              }`}
            />
            <span className="text-xs sm:text-sm font-mono text-[#ede8df]">
        ...
        {/* Navigation Tabs */}
        <div className="flex items-center gap-2.5 border-b border-white/[0.06] pb-4 overflow-x-auto scrollbar-none">
          <button
            onClick={() => setActiveTab("policies")}
            className={`px-5 py-2.5 rounded-full text-xs sm:text-sm font-medium flex items-center gap-2.5 transition-all ${
              activeTab === "policies"
        ...
        {activeTab === "policies" && (
          <div className="max-w-4xl space-y-6">
            <form onSubmit={handleSavePolicies} className="hirael-card p-6 sm:p-8 space-y-6 rounded-2xl sm:rounded-3xl">
              <div>
                <h3 className="text-base sm:text-lg font-medium text-[#ede8df] flex items-center gap-2.5 mb-1.5">
                  <Shield className="w-5 h-5 text-[#d4a373]" />
                  <span>Deterministic Quality &amp; Review Controls</span>
                </h3>
                <p className="text-xs sm:text-sm text-[#8a8a93]">
                  Enforce strict verification rules before variants can be scheduled or dispatched.
                </p>
              </div>
              ...
              <div className="space-y-5 divide-y divide-white/[0.06]">
                <div className="pt-4 flex items-center justify-between gap-4">
                  <div>
                    <span className="font-medium text-[#ede8df] text-sm sm:text-base block">
                      Strict Quality Assurance Threshold (QA score &ge; 90%)
                    </span>
                    <span className="text-xs sm:text-sm text-[#71717a] mt-0.5 block">
                  ...
                  <input
                    ...
                    className="w-5 h-5 rounded text-[#d4a373] focus:ring-0 border-white/20 bg-black cursor-pointer shrink-0"
                  />
                </div>
                ...
                <div className="pt-5 space-y-3">
                  <div className="flex items-center justify-between">
                    <label className="font-medium text-[#ede8df] text-sm sm:text-base block">
                    ...
                    <span className="px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-xs font-mono flex items-center gap-1.5">
                      <Sparkles className="w-3.5 h-3.5" /> Groq Active (Primary)
                    </span>
                  </div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-black/60 border border-white/10 text-[#ede8df] text-xs sm:text-sm focus:outline-none focus:border-[#d4a373]/60 cursor-pointer"
                  >
              ...
              <div className="pt-5 border-t border-white/[0.08] flex items-center justify-between">
                {policySaved ? (
                  <span className="text-xs sm:text-sm font-medium text-emerald-400 flex items-center gap-2 font-mono">
                    <CheckCircle2 className="w-4 h-4" /> Policies updated successfully!
                  </span>
                ) : (
                  <span className="text-xs sm:text-sm text-[#71717a] font-mono">Changes apply immediately to new jobs.</span>
                )}
                <button
                  type="submit"
                  className="hirael-pill-btn text-xs sm:text-sm py-2.5 sm:py-3 px-6 font-semibold"
                >
        ...
        {activeTab === "telemetry" && (
          <div className="hirael-card p-6 sm:p-8 space-y-5 rounded-2xl sm:rounded-3xl">
            <h3 className="text-base sm:text-lg font-medium text-[#ede8df] flex items-center gap-2.5">
              <Cpu className="w-5 h-5 text-[#d4a373]" />
              <span>Agent Workflow Execution &amp; Latency Traces</span>
            </h3>

            {agentRuns.length === 0 ? (
              <p className="text-xs sm:text-sm text-[#71717a] py-12 text-center font-mono">
                No agent runs recorded yet. Generate variants from the Content Studio to inspect traces.
              </p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs sm:text-sm text-[#a6a39b]">
                  <thead className="text-[11px] sm:text-xs uppercase font-mono text-[#787672] bg-white/[0.02] border-b border-white/[0.06]">
                    <tr>
                  ...
                  <tbody className="divide-y divide-white/[0.04] font-mono">
                    {agentRuns.map((run) => (
                      <tr key={run.id} className="hover:bg-white/[0.02] transition-colors">
                        <td className="py-3.5 px-4 font-medium text-[#ede8df]">
                          {run.agent_name}
                        </td>
                        <td className="py-3.5 px-4">
                          <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-xs">
                            {run.status}
                          </span>
                        </td>
                        <td className="py-3.5 px-4 text-[#8a8a93]">{run.model}</td>
                        <td className="py-3.5 px-4 text-[#d4a373] font-bold">{run.latency_ms}ms</td>
                        <td className="py-3.5 px-4 text-[#8a8a93]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="hirael-card p-6 sm:p-8 space-y-5 rounded-2xl sm:rounded-3xl">
              <h3 className="text-base sm:text-lg font-medium text-[#ede8df] flex items-center gap-2.5">
                ...
                <span>AI Agent Pipeline Health</span>
              </h3>
              <div className="space-y-3">
                {Object.entries(systemHealth?.agents || {}).map(([agent, status]) => (
                  <div
                    key={agent}
                    className="p-4 rounded-xl bg-white/[0.02] border border-white/[0.05] flex items-center justify-between text-xs sm:text-sm"
                  >
                    <span className="font-medium text-[#ede8df] capitalize">
                      {agent.replace("_", " ")}
                    </span>
                    <span className="px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-xs font-mono uppercase">
                      {status}
                    </span>
                  </div>
                ))}
            ...
            <div className="hirael-card p-6 sm:p-8 space-y-5 rounded-2xl sm:rounded-3xl">
              <h3 className="text-base sm:text-lg font-medium text-[#ede8df] flex items-center gap-2.5">
                <Server className="w-5 h-5 text-[#ede8df]" />
                <span>Registered Platform Adapters</span>
              </h3>
              <div className="space-y-3">
                  ...
                  ([plat, details]: [string, any]) => (
                    ...
                    >
                      <div>
                        ...
                        <span className="text-xs text-[#71717a] font-mono mt-0.5 block">
        ...
        {activeTab === "audit" && (
          <div className="hirael-card p-6 sm:p-8 space-y-5 rounded-2xl sm:rounded-3xl">
            ...
            {auditLogs.length === 0 ? (
              ...
              <div className="divide-y divide-white/[0.05] font-mono text-xs sm:text-sm">
                {auditLogs.map((log) => (
                  <div key={log.id} className="py-4 flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      <span className="px-2.5 py-1 rounded-md bg-white/[0.05] text-[#ede8df] border border-white/[0.08] text-xs uppercase">
                        {log.resource_type}
                      </span>
