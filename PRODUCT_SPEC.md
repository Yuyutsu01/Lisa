...
### 23.1 Core Operating Invariant
1. **AI Proposes, Software Validates, Human Approves:**
   - AI agents reason and generate structured variants.
   - Deterministic application software enforces Pydantic schemas, character limits, PII protection, and platform ToS policies.
   - Humans review and authorize before any publishing event occurs, or establish explicit, auditable 30-day Trusted Automation rules.

### 23.2 The 8 AI Harnessing Guardrail Layers (Reference: docs/AI_HARNESSING_AND_GUARDRAILS.md)
1. **Prompt Injection Defense & Untrusted Boundaries (FR-BRAND-005):**
   - All user-supplied text, uploaded documents, scraped URLs, and RAG context are encapsulated in `<untrusted_content>` boundary tags.
   - System prompts enforce strict anti-override rules: embedded instructions inside data tags must be ignored.
   - Deterministic pre-check scanner detects injection signatures (`ignore previous instructions`, `###override`, `you are now DAN`) and sets `injection_risk_flag=True` for human audit.
2. **Output Schema Validation Layer:**
   - Every agent output is strictly validated against Pydantic models with bounded 1-retry on malformed responses.
3. **Independent Policy Validation Gate:**
   - Scans variant copy for PII leakage (unredacted emails, phones, SSNs), platform ToS violations (Meta/Instagram engagement-bait comment gating), and brand forbidden phrases.
   - Failing variants are assigned `VariantStatus.POLICY_FLAGGED` and blocked from review until human override.
4. **Semantic Hallucination & Fact-Grounding Guardrail:**
   - QA Agent isolates quantitative claims (percentages, numbers, latency figures, scale benchmarks) and verifies grounding against canonical source facts.
   - Unverified claims trigger critical `QualityIssue`, cap quality score $\le 0.50$, and strictly block auto-approval.
5. **Human-In-The-Loop (HITL) & Trusted Automation Gate:**
   - Publishing Orchestrator blocks publishing unless the variant is manually approved (`approved_by`/`approved_at`) or authorized by an active, unexpired 30-day `TrustedAutomationRule`.
6. **Token Budget & Cost Ceilings:**
   - Per-job token ceilings (15k tokens) and per-workspace sliding-window rate limiters prevent runaway revision loops. Exceeding ceilings raises `BudgetExceededError` and marks variants `budget_exceeded`.
7. **Fabricated-Success & Provenance Invariant:**
   - Systems never fabricate live URLs or metric counts. Video platforms without direct API upload use **Mode C (Export/Manual Handoff)** with status `exported`.
   - All metric records carry `metrics_source` / `data_provenance` tags (`platform_api`, `simulated`, `unavailable`).
8. **Circuit Breakers & Fallback Markers:**
   - Explicit timeouts (30s API / 75s generation) with bounded transient retries.
   - Offline fallback content stamps `is_fallback: true` to prevent confusing offline templates with live AI generations.

### 23.3 Observability & Adversarial CI Suite
- Continuous red-team test suite (`backend/tests/test_guardrails_redteam.py`) runs on every commit to verify that all guardrails successfully intercept adversarial injection, hallucination, and policy breach attempts.
- Every agent run records token usage, latency, and execution telemetry in `agent_runs` and `audit_logs`.
