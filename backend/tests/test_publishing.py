...
@pytest.mark.asyncio
async def test_unsupported_platform_publishing_error(client: AsyncClient):
    """
    Assert that attempting to publish to a decommissioned/unsupported platform fails cleanly.
    """
    res = await client.post(
        "/api/v1/auth/register",
        json={"email": "tester@domain.com", "name": "Platform Tester", "password": "Password123!"},
    )
    token = res.json()["token"]["access_token"]
    ws_id = res.json()["workspace_id"]
    headers = {"Authorization": f"Bearer {token}"}

    # 1. Create Source and Generate LinkedIn Variant
    src_res = await client.post(
        f"/api/v1/workspaces/{ws_id}/sources",
        headers=headers,
        json={
    ...
    # 3. Modify platform to unknown and attempt publish
    # Using service directly with unsupported platform
    from app.publishing.registry import AdapterRegistry
    assert AdapterRegistry.get_adapter("youtube") is None
